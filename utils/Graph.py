...
def Plot_Graphes_for_fog (id,epochs,num_users,accTrain,accTest,lossTrain,lossTest):
    ...
    clients_accuracy_train=[[0 for _ in range(epochs+1)] for _ in range(num_users)]
    clients_accuracy_test=[[0 for _ in range(epochs+1)] for _ in range(num_users)] 
    clients_loss_train=[[0 for _ in range(epochs+1)] for _ in range(num_users)]
    clients_loss_test=[[0 for _ in range(epochs+1)] for _ in range(num_users)]
